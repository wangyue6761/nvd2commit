...
def find_unavailable_urls(urls):
    ...
    sleeptime = 0
    unavailable_urls = []
    cf.logger.info(f'Len(urls) = {len(urls)}')
    for url in urls:  
        response = requests.head("https://www.baidu.com/") # 先初始化一个response值
        try: 
            # response = requests.head(url)
            response = requests.get(url=url,timeout=10)
            print("URL:{}   Respone:{}".format(url, response.status_code))
        except:
            unavailable_urls.append(url)
            print("FAIL URL:{}   Respone:{}".format(url, "Time out"))
...
def get_ref_links():
    ...
    if db.table_exists('fixes'):
        ...
        df_fixes = extract_project_links(df_master) # df_fixes包含 cve_id,hash,repo_url三列，前五列内容正常
...
def store_tables(df_fixes):
    ...
    for repo_url in repo_urls:
        ...
        try:
            ...
            df_commit, df_file, df_method = extract_commits(repo_url, hashes) # 注意到一个fixes url 可能对应多个commit hashes 修复提交
    ...
    if db.table_exists('commits'):
        ...
        cf.logger.info(f'Number of commits retrieved from all the repos: {commit_count}')
    else:
    ...
    if db.table_exists('file_change'):
        ...
        cf.logger.info(f'Number of files changed by all the commits: {file_count}')
    else:
    ...
    if db.table_exists('method_change'):
        ...
        cf.logger.info(f'Number of total methods fetched by all the commits: {method_count}')

        vul_method_count = \
        pd.read_sql('SELECT count(*) from method_change WHERE before_change="True";', con=db.conn).iloc[0].iloc[0]
        cf.logger.info(f"Number of vulnerable methods fetched by all the commits: {vul_method_count}")
    else:
